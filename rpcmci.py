...
# Authors: Elena Saggioro, Oana Popescu, Wiebke Günther, Sagar Simha, Matthias Bruhns, Jakob Runge <jakob@jakob-runge.com>
#
...
class RPCMCI(PCMCI):
    r"""RPCMCI class for extracting causal regimes and the associated graphs from
        ...
        initial_assignment: matrix indicating which regimes are active
            Used as initial solution for the solver to guide towards the optimal
            solution, if such a solution is available. Must be a Numpy matrix
    ...
    def run_rpcmci(self,
        ...
        link_assumprtions : dict
            Dictionary with link assumptions
        ...
        def one_annealing_step(a):
            ...
            if self.initial_assignment is None:
                ...
                gamma_opt = self.initial_assignment
                ...
                for k in range(num_regimes):
                    ...
                    try:
                        ...
                        predicted = pred.predict(
                            ...
                            new_data=DataFrame(np.where(np.isnan(data), 999., data), missing_flag=self.missing_flag)
                        )

                        if len(predicted.shape) < 2:
                            # find minimum among lists
                            len_lists = [len(lst) for lst in predicted]
                            min_length = min(len_lists)
                            # Trim each list to the minimum length
                            trimmed_lists = [lst[:min_length] for lst in predicted]

                            # Create a numpy array from the trimmed lists
                            pred_list = np.array(trimmed_lists)
                            predicted = np.array(pred_list).transpose()
                            ...
                            for target in range(self.N):
                                ...
                                original_data[:, target] = pred.get_test_array(target)[0][:min_length].flatten()
                        else:
                            original_data = np.zeros(predicted.shape)
                            for target in range(self.N):
                                # print(data.shape, predicted.shape, original_data.shape, pred.get_test_array(target).shape, mask_of_k.sum(axis=0))
                    ...
                    # residuals
                    orig_residuals = original_data - predicted
                    if residuals[k, int(tau_max):, :].shape == orig_residuals.shape:
                        residuals[k, int(tau_max):, :] = original_data - predicted
                    else:
                        """target_shape = residuals[k, int(tau_max):, :].shape
                        # Calculate padding needed
                        padding = [(0, max(0, target_shape[i] - orig_residuals.shape[i])) for i in
                                   range(len(target_shape))]

                        # Apply padding
                        padded_array = np.pad(orig_residuals, padding, mode='constant', constant_values=0)

                        # Print the result
                        residuals[k, int(tau_max):, :] = padded_array

                        # print(np.abs(residuals[k, int(tau_max):, :]).mean(axis=0))"""
                        target_T = T - int(tau_max)
                        padded_array = np.zeros((target_T, self.N))
                        s = 0
                        for t in range(T-int(tau_max)):
                            conditions = [not np.isnan(data[t-tau,0]) for tau in range(int(tau_max)+1)]
                            if np.all(conditions):
                                padded_array[t,:] = original_data[s,:] - predicted[s,:]
                                s += 1
